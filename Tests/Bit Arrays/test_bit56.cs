using NUnit.Framework;
using MaxMath;

using static Unity.Mathematics.math;
using static MaxMath.maxmath;

namespace BitCollections.Tests
{
    internal static class _bit56
    {
        internal static bit56 TestData_LHS => new bit56{ Bits = 0b1011_0001_1101_0010_0010_1011_1001_1110_1101_0110_0001_1101_1011_0101_1101_1110 };
        internal static bit56 TestData_RHS => new bit56{ Bits = 0b1110_1100_1011_0001_1010_1111_1010_1010_1001_1011_1101_0001_1011_0110_0000_0110 };

        private static Random64 rng = Random64.New;

        private static bit56 Random => new bit56 { Bits = rng.NextULong() };
        private static bool RandomBool => rng.NextBool();
        private static int RandomIndex() => (int)rng.NextLong(0, 56);
        private static int RandomNumBits(int index) => (int)rng.NextLong(0, 56 - index + 1);

        private static void AssertBitsOutsideFieldAreUntouched(bit56 t0, bit56 t1, int idx, int num)
        {
            for (int j = 0; j < idx; j++)
            {
                Assert.AreEqual(t0[j], t1[j]);
            }
            for (int j = idx + num; j < 56; j++)
            {
                Assert.AreEqual(t0[j], t1[j]);
            }
        }

        [Test]
        public static void Get()
        {
            bool result = true;

            result &= TestData_LHS[0] == false;
            result &= TestData_LHS[1] == true;
            result &= TestData_LHS[2] == true;
            result &= TestData_LHS[3] == true;

            result &= TestData_LHS[4] == true;
            result &= TestData_LHS[5] == false;
            result &= TestData_LHS[6] == true;
            result &= TestData_LHS[7] == true;

            result &= TestData_LHS[8] == true;
            result &= TestData_LHS[9] == false;
            result &= TestData_LHS[10] == true;
            result &= TestData_LHS[11] == false;

            result &= TestData_LHS[12] == true;
            result &= TestData_LHS[13] == true;
            result &= TestData_LHS[14] == false;
            result &= TestData_LHS[15] == true;

            result &= TestData_LHS[16] == true;
            result &= TestData_LHS[17] == false;
            result &= TestData_LHS[18] == true;
            result &= TestData_LHS[19] == true;

            result &= TestData_LHS[20] == true;
            result &= TestData_LHS[21] == false;
            result &= TestData_LHS[22] == false;
            result &= TestData_LHS[23] == false;

            result &= TestData_LHS[24] == false;
            result &= TestData_LHS[25] == true;
            result &= TestData_LHS[26] == true;
            result &= TestData_LHS[27] == false;

            result &= TestData_LHS[28] == true;
            result &= TestData_LHS[29] == false;
            result &= TestData_LHS[30] == true;
            result &= TestData_LHS[31] == true;

            result &= TestData_LHS[32] == false;
            result &= TestData_LHS[33] == true;
            result &= TestData_LHS[34] == true;
            result &= TestData_LHS[35] == true;

            result &= TestData_LHS[36] == true;
            result &= TestData_LHS[37] == false;
            result &= TestData_LHS[38] == false;
            result &= TestData_LHS[39] == true;

            result &= TestData_LHS[40] == true;
            result &= TestData_LHS[41] == true;
            result &= TestData_LHS[42] == false;
            result &= TestData_LHS[43] == true;

            result &= TestData_LHS[44] == false;
            result &= TestData_LHS[45] == true;
            result &= TestData_LHS[46] == false;
            result &= TestData_LHS[47] == false;

            result &= TestData_LHS[48] == false;
            result &= TestData_LHS[49] == true;
            result &= TestData_LHS[50] == false;
            result &= TestData_LHS[51] == false;

            result &= TestData_LHS[52] == true;
            result &= TestData_LHS[53] == false;
            result &= TestData_LHS[54] == true;
            result &= TestData_LHS[55] == true;


            Assert.AreEqual(result, true);
        }

        [Test]
        public static void Set()
        {
            bool result = true;

            for (int i = 0; i < 56; i++)
            {
                bit56 test = TestData_LHS;
                test[i] = false;
                result &= test.Bits == (TestData_LHS.Bits & ~(1ul << i));
            }

            for (int i = 0; i < 56; i++)
            {
                bit56 test = TestData_LHS;
                test[i] = true;
                result &= test.Bits == (TestData_LHS.Bits | (1ul << i));
            }

            Assert.AreEqual(result, true);
        }

        
        [Test]
        public static void SetReplicate()
        {
            for (int i = 0; i < 128; i++)
            {
                bool b = RandomBool;
                bit56 t = Random.SetReplicate(b);

                for (int j = 0; j < 56; j++)
                {
                    Assert.AreEqual(b, t[j]);
                }
            }
        }

        [Test]
        public static void SetReplicate_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                bool b = RandomBool;
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t0 = Random;
                bit56 t1 = t0.SetReplicate(idx, num, b);

                AssertBitsOutsideFieldAreUntouched(t0, t1, idx, num);
                for (int j = 0; j < num; j++)
                {
                    Assert.AreEqual(t1[j + idx], b);
                }
            }
        }

        [Test]
        public static void ResetFirst()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t0 = Random;
                bit56 t1 = t0.ResetFirst();

                int iof = t0.IndexOfFirst();
                for (int j = 0; j < 56; j++)
                {
                    if (j == iof)
                    {
                        Assert.IsFalse(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void ResetFirst_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t0 = Random;
                bit56 t1 = t0.ResetFirst(idx, num);

                int iof = t0.IndexOfFirst(idx, num);
                for (int j = 0; j < 56; j++)
                {
                    if (j == iof)
                    {
                        Assert.IsFalse(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void SetFirst()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t0 = Random;
                bit56 t1 = t0.SetFirst();

                int iof = (!t0).IndexOfFirst();
                for (int j = 0; j < 56; j++)
                {
                    if (j == iof)
                    {
                        Assert.IsTrue(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void SetFirst_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t0 = Random;
                bit56 t1 = t0.SetFirst(idx, num);

                int iof = (!t0).IndexOfFirst(idx, num);
                for (int j = 0; j < 56; j++)
                {
                    if (j == iof)
                    {
                        Assert.IsTrue(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void ResetLast()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t0 = Random;
                bit56 t1 = t0.ResetLast();

                int iof = t0.IndexOfLast();
                for (int j = 0; j < 56; j++)
                {
                    if (j == iof)
                    {
                        Assert.IsFalse(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void ResetLast_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t0 = Random;
                bit56 t1 = t0.ResetLast(idx, num);

                int iof = t0.IndexOfLast(idx, num);
                for (int j = 0; j < 56; j++)
                {
                    if (j == iof)
                    {
                        Assert.IsFalse(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void SetLast()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t0 = Random;
                bit56 t1 = t0.SetLast();

                int iof = (!t0).IndexOfLast();
                for (int j = 0; j < 56; j++)
                {
                    if (j == iof)
                    {
                        Assert.IsTrue(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void SetLast_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t0 = Random;
                bit56 t1 = t0.SetLast(idx, num);

                int iof = (!t0).IndexOfLast(idx, num);
                for (int j = 0; j < 56; j++)
                {
                    if (j == iof)
                    {
                        Assert.IsTrue(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void ShiftLeft()
        {
            for (int i = 0; i < 128; i++)
            {
                int n = RandomNumBits(0) % 56;
                bit56 t0 = Random;
                bit56 t1 = t0.ShiftLeft(n);

                for (int j = 0; j < 56; j++)
                {
                    if (j < n)
                    {
                        Assert.IsFalse(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j - n], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void ShiftLeft_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                int n = (int)rng.NextLong(0, num + 1);
                bit56 t0 = Random;
                bit56 t1 = t0.ShiftLeft(idx, num, n);
                
                AssertBitsOutsideFieldAreUntouched(t0, t1, idx, num);
                for (int j = 0; j < n; j++)
                {
                    Assert.IsFalse(t1[j + idx]);
                }
                for (int j = n; j < num - n; j++)
                {
                    Assert.AreEqual(t0[j + idx - n], t1[j + idx]);
                }
            }
        }

        [Test]
        public static void ShiftRight()
        {
            for (int i = 0; i < 128; i++)
            {
                int n = RandomNumBits(0) % 56;
                bit56 t0 = Random;
                bit56 t1 = t0.ShiftRight(n);

                for (int j = 0; j < 56; j++)
                {
                    if (j >= 56 - n)
                    {
                        Assert.IsFalse(t1[j]);
                    }
                    else
                    {
                        Assert.AreEqual(t0[j + n], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void ShiftRight_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                int n = (int)rng.NextLong(0, num + 1);
                bit56 t0 = Random;
                bit56 t1 = t0.ShiftRight(idx, num, n);
                
                AssertBitsOutsideFieldAreUntouched(t0, t1, idx, num);
                for (int j = idx; j < idx + num - n; j++)
                {
                    Assert.AreEqual(t0[j + n], t1[j]);
                }
                for (int j = idx + num - n; j < idx + num; j++)
                {
                    Assert.IsFalse(t1[j]);
                }
            }
        }

        [Test]
        public static void RotateLeft()
        {
            for (int i = 0; i < 128; i++)
            {
                int n = RandomNumBits(0);
                bit56 t0 = Random;
                bit56 t1 = t0.RotateLeft(n);

                for (int j = 0; j < 56; j++)
                {
                    Assert.AreEqual(t0[(j - n + 56) % 56], t1[j]);
                }
            }
        }

        [Test]
        public static void RotateLeft_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                int n = (int)rng.NextLong(0, num + 1);
                bit56 t0 = Random;
                bit56 t1 = num == 0 ? t0 : t0.RotateLeft(idx, num, n);
                
                AssertBitsOutsideFieldAreUntouched(t0, t1, idx, num);
                if (n == 0)
                {
                    Assert.AreEqual(t0.Bits, t1.Bits);
                }
                else
                {
                    for (int j = 0; j < num; j++)
                    {
                        int srcInField = (j - n + num) % num;
                        int srcIndex = idx + srcInField;
                        int dstIndex = idx + j;

                        Assert.AreEqual(t0[srcIndex], t1[dstIndex]);
                    }
                }
            }
        }

        [Test]
        public static void RotateRight()
        {
            for (int i = 0; i < 128; i++)
            {
                int n = RandomNumBits(0);
                bit56 t0 = Random;
                bit56 t1 = t0.RotateRight(n);

                for (int j = 0; j < 56; j++)
                {
                    Assert.AreEqual(t0[(j + n + 56) % 56], t1[j]);
                }
            }
        }

        [Test]
        public static void RotateRight_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                int n = (int)rng.NextLong(0, num + 1);
                bit56 t0 = Random;
                bit56 t1 = num == 0 ? t0 : t0.RotateRight(idx, num, n);
                
                AssertBitsOutsideFieldAreUntouched(t0, t1, idx, num);
                if (n == 0)
                {
                    Assert.AreEqual(t0.Bits, t1.Bits);
                }
                else
                {
                    for (int j = 0; j < num; j++)
                    {
                        int srcInField = (j + n + num) % num;
                        int srcIndex = idx + srcInField;
                        int dstIndex = idx + j;

                        Assert.AreEqual(t0[srcIndex], t1[dstIndex]);
                    }
                }
            }
        }

        [Test]
        public static void Reverse()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t0 = Random;
                bit56 t1 = t0.Reverse();

                for (int j = 0; j < 56; j++)
                {
                    Assert.AreEqual(t0[(56 - 1) - j], t1[j]);
                }
            }
        }

        [Test]
        public static void Reverse_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t0 = Random;
                bit56 t1 = t0.Reverse(idx, num);
                
                AssertBitsOutsideFieldAreUntouched(t0, t1, idx, num);
                for (int j = 0; j < num; j++)
                {
                    Assert.AreEqual(t0[(num - 1) - j + idx], t1[j + idx]);
                }
            }
        }

        [Test]
        public static void Swap()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx0 = RandomIndex();
                int idx1 = RandomIndex();
                bit56 t0 = Random;
                bit56 t1 = t0.Swap(idx0, idx1);
                
                Assert.AreEqual(t0[idx0], t1[idx1]);
                Assert.AreEqual(t0[idx1], t1[idx0]);

                for (int j = 0; j < 56; j++)
                {
                    if (j != idx0
                     && j != idx1)
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }

        [Test]
        public static void Swap_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int num = (int)rng.NextLong(0, (56 / 2) + 1);
                int idx0 = (int)rng.NextLong(0, 56 - 2 * num + tobyte(num != 0));
                int idx1 = (int)rng.NextLong(idx0 + num, 56 - num + tobyte(num != 0));
                bit56 t0 = Random;
                bit56 t1 = t0.Swap(idx0, idx1, num);

                for (int j = 0; j < num; j++)
                {
                    Assert.AreEqual(t0[j + idx0], t1[j + idx1]);
                    Assert.AreEqual(t0[j + idx1], t1[j + idx0]);
                }

                for (int j = 0; j < 56; j++)
                {
                    if (!isinrange(j, idx0, idx0 + num - 1)
                     && !isinrange(j, idx1, idx1 + num - 1))
                    {
                        Assert.AreEqual(t0[j], t1[j]);
                    }
                }
            }
        }
        
        [Test]
        public static void Flip()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t0 = Random;
                bit56 t1 = t0.Flip();

                for (int j = 0; j < 56; j++)
                {
                    Assert.AreEqual(t1[j], !t0[j]);
                }
            }
        }

        [Test]
        public static void Flip_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t0 = Random;
                bit56 t1 = t0.Flip(idx, num);

                AssertBitsOutsideFieldAreUntouched(t0, t1, idx, num);
                for (int j = 0; j < num; j++)
                {
                    Assert.AreEqual(t1[j + idx], !t0[j + idx]);
                }
            }
        }
        
        [Test]
        public static void IndexOfFirst()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t = Random;
                int iof = t.IndexOfFirst();

                if (iof != 64)
                {
                    Assert.IsTrue(t[iof]);
                }
                for (int j = 0; j < min(iof, 56); j++)
                {
                    Assert.IsFalse(t[j]);
                }
            }
        }

        [Test]
        public static void IndexOfFirst_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t = Random;
                int iof = t.IndexOfFirst(idx, num);

                if (num == 0)
                {
                    Assert.IsFalse(isinrange(iof, 0, 55));
                    break;
                }

                if (iof != 64)
                {
                    Assert.IsTrue(t[iof]);
                }
                for (int j = idx; j < min(iof, idx + num); j++)
                {
                    Assert.IsFalse(t[j]);
                }
            }
        }
        
        [Test]
        public static void IndexOfLast()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t = Random;
                int iol = t.IndexOfLast();

                if (iol >= 0)
                {
                    Assert.IsTrue(t[iol]);
                    for (int j = 55; j > iol; j--)
                    {
                        Assert.IsFalse(t[j]);
                    }
                }
                else
                {
                    for (int j = 55; j >= 0; j--)
                    {
                        Assert.IsFalse(t[j]);
                    }
                }
            }
        }

        [Test]
        public static void IndexOfLast_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t = Random;
                int iol = t.IndexOfLast(idx, num);
                
                if (num == 0)
                {
                    Assert.IsFalse(isinrange(iol, 0, 55));
                    break;
                }

                if (iol >= 0)
                {
                    Assert.IsTrue(t[iol]);
                    for (int j = idx + num - 1; j > iol; j--)
                    {
                        Assert.IsFalse(t[j]);
                    }
                }
                else
                {
                    for (int j = idx + num - 1; j >= idx; j--)
                    {
                        Assert.IsFalse(t[j]);
                    }
                }
            }
        }

        private static int __countbits(bit56 t, int idx, int num)
        {
            int r = 0;
            for (int i = 0; i < num; i++)
            {
                r += tobyte(t[i + idx]);
            }
            return r;
        }
        
        [Test]
        public static void CountBits()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t = Random;

                Assert.AreEqual(t.CountBits(), __countbits(t, 0, 56));
            }
        }

        [Test]
        public static void CountBits_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t = Random;
                
                Assert.AreEqual(t.CountBits(idx, num), __countbits(t, idx, num));
            }
        }
        
        private static int __longeststring(bool value, bit56 t, int idx, int num)
        {
            if (num <= 0) 
            {
                return 0;
            }

            int maximum = 0;
            int current = 0;
            for (int i = idx; i < idx + num; i++)
            {
                if (t[i] == value)
                {
                    maximum = max(++current, maximum);
                }
                else
                {
                    current = 0;
                }
            }

            return maximum;
        }

        [Test]
        public static void FindString()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t = Random;
                int count = (int)rng.NextLong(0, 56 + 1);

                int iof = t.FindString(true, count);
                if (iof != 64)
                {
                    if (count != 0)
                    {
                        Assert.IsTrue(__longeststring(true, t, 0, iof) < count);
                    }
                    for (int j = 0; j < count; j++)
                    {
                        Assert.IsTrue(t[j + iof]);
                    }
                }

                iof = t.FindString(false, count);
                if (iof != 64)
                {
                    if (count != 0)
                    {
                        Assert.IsTrue(__longeststring(false, t, 0, iof) < count);
                    }
                    for (int j = 0; j < count; j++)
                    {
                        Assert.IsFalse(t[j + iof]);
                    }
                }
            }
        }

        [Test]
        public static void FindString_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t = Random;
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                int count = (int)rng.NextLong(0, num + 1);

                int iof = t.FindString(idx, num, true, count);
                if (iof != 64)
                {
                    if (count != 0)
                    {
                        Assert.IsTrue(__longeststring(true, t, idx, iof - idx) < count);
                    }
                    for (int j = 0; j < count; j++)
                    {
                        Assert.IsTrue(t[j + iof]);
                    }
                }
                
                iof = t.FindString(idx, num, false, count);
                if (iof != 64)
                {
                    if (count != 0)
                    {
                        Assert.IsTrue(__longeststring(false, t, idx, iof - idx) < count);
                    }
                    for (int j = 0; j < count; j++)
                    {
                        Assert.IsFalse(t[j + iof]);
                    }
                }
            }
        }
        
        [Test]
        public static void TestAll()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t = Random;

                Assert.AreEqual(t.TestAll(), __countbits(t, 0, 56) == 56);
            }
        }

        [Test]
        public static void TestAll_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t = Random;

                Assert.AreEqual(t.TestAll(idx, num), __countbits(t, idx, num) == num);
            }
        }
        
        [Test]
        public static void TestAny()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t = Random;

                Assert.AreEqual(t.TestAny(), __countbits(t, 0, 56) != 0);
            }
        }

        [Test]
        public static void TestAny_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t = Random;

                Assert.AreEqual(t.TestAny(idx, num), __countbits(t, idx, num) != 0);
            }
        }
        
        [Test]
        public static void TestNone()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t = Random;

                Assert.AreEqual(t.TestNone(), __countbits(t, 0, 56) == 0);
            }
        }

        [Test]
        public static void TestNone_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t = Random;

                Assert.AreEqual(t.TestNone(idx, num), __countbits(t, idx, num) == 0);
            }
        }
        
        [Test]
        public static void TestNotAll()
        {
            for (int i = 0; i < 128; i++)
            {
                bit56 t = Random;

                Assert.AreEqual(t.TestNotAll(), __countbits(t, 0, 56) != 56);
            }
        }

        [Test]
        public static void TestNotAll_BitField()
        {
            for (int i = 0; i < 128; i++)
            {
                int idx = RandomIndex();
                int num = RandomNumBits(idx);
                bit56 t = Random;

                Assert.AreEqual(t.TestNotAll(idx, num), __countbits(t, idx, num) != num);
            }
        }
    }
}