...
namespace BitCollections.Tests
{
    internal static class _UInt2x32
    {
        ...
        private static UInt2x32 Random(byte min = 0, byte max = 0)
        {
            Random8 rng = Random8.New;

            UInt2x32 test = new UInt2x32();
            for (int i = 0; i < 32; i++)
            {
        ...
        {
            UInt2x32 test = Random();
            byte32 byte32test = test;

            for (int i = 0; i < 32; i++)
            {
                Assert.AreEqual((uint)test[i], (uint)byte32test[i]);
            }

            test = (UInt2x32)byte32test;
