...
namespace BitCollections
{
    ...
    unsafe public struct bit8 : IArray<bool>, IEquatable<bit8>
    {
        ...
        {
            intern = (byte)((uint)maxmath.touint8(value) * byte.MaxValue);
        }
...
Assert.IsWithinArrayBounds(index + 7, values.Length);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bit8 operator == (bit8 lhs, bit8 rhs)
        {
            return !(lhs != rhs);
        }
        ...
        public static bit8 operator & (bit8 lhs, bit8 rhs)
        {
            lhs.intern = (byte)(lhs.intern & rhs.intern);
...
Assert.IsWithinArrayBounds(index, Length);
        ...
        {
            NativeArray<bool> result = new NativeArray<bool>(8, allocator);
...
Assert.IsValidSubarray(index, numBits, Length);
        ...
        {
            for (int2 i = new int2(7, Length); i.x > 0; i--)
            {
...
Assert.IsValidSubarray(index, numBits, Length);
        ...
        {
            intern = value ? byte.MaxValue : byte.MinValue;
        }
