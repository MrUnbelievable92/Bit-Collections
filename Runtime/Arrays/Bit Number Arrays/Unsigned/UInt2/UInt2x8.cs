...
{
    [Serializable]
    unsafe public struct UInt2x8 : IBitNumberArray<uint>, IEquatable<UInt2x8>
    {
        internal ushort intern;


        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public UInt2x8(uint x0, uint x1, uint x2, uint x3, uint x4, uint x5, uint x6, uint x7) : this(new uint8(x0, x1, x2, x3, x4, x5, x6, x7))
        { }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public UInt2x8(uint x0_7) : this(new uint8(x0_7))
        { }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public UInt2x8(uint8 x0_7)
        {
...
Assert.IsNotGreater(x0_7.x7, UInt2.MaxValue);
            ...
            intern = (ushort)maxmath.csum(maxmath.shl(x0_7, (uint)new UInt2x8().BitsPerNumber * new uint8(0u, 1u, 2u, 3u, 4u, 5u, 6u, 7u)));
        }
...
Assert.IsValidSubarray(index, numNumbers, Length);
            ...
            if (Constant.IsConstantExpression(value))
            {
                ...
                else
                {
                    ...
                    uint newValues = new UInt2x8(value).intern & mask;
