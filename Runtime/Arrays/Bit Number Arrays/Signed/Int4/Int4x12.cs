...
{
    [Serializable]
    unsafe public struct Int4x12 : IBitNumberArray<int>, IEquatable<Int4x12>
    {
        internal UInt48 intern;


        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int4x12(int x0, int x1, int x2, int x3, int x4, int x5, int x6, int x7, int x8, int x9, int x10, int x11) : this(new int8(x0, x1, x2, x3, x4, x5, x6, x7), new int4(x8, x9, x10, x11))
        { }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int4x12(int x0_11)
        {
            int8 vector = x0_11;

            this = new Int4x12(vector, vector.v4_0);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int4x12(int8 x0_7, int4 x8_11)
        {
...
Assert.IsBetween(x8_11.x, Int4.MinValue, Int4.MaxValue);
Assert.IsBetween(x8_11.y, Int4.MinValue, Int4.MaxValue);
Assert.IsBetween(x8_11.z, Int4.MinValue, Int4.MaxValue);
Assert.IsBetween(x8_11.w, Int4.MinValue, Int4.MaxValue);

            int8 mask = (int)maxmath.bitmask32(new Int4x12().BitsPerNumber);

            intern = (UInt48)((ulong)(uint)maxmath.csum(maxmath.shl(mask & x0_7, new Int4x12().BitsPerNumber * new int8(0, 1, 2, 3, 4, 5, 6, 7)))
                             |
                             ((ulong)(uint)maxmath.csum(maxmath.shl(mask & new int8(x8_11, default(int4)), new Int4x12().BitsPerNumber * new int8(0, 1, 2, 3, 4, 5, 6, 7))) << (new Int4x12().BitsPerNumber * 8)));
        }
...
Assert.IsWithinArrayBounds(index, Length);
                ...
                {
                    return (int)(((long)(Int48)intern) >> (index * BitsPerNumber));
                }
                else
                {
...
Assert.IsBetween(value.w, MinValue, MaxValue);
            ...
            intern = (UInt48)((long)maxmath.andnot((ulong)intern, (ulong)maxmath.bitmask64(4 * BitsPerNumber,   index * BitsPerNumber))
                             |
...
Assert.IsValidSubarray(index, numNumbers, Length);
            ...
            if (Constant.IsConstantExpression(value))
            {
                ...
                else
                {
                    ...
                    ulong newValues = new Int4x12(value).intern & mask;
