...
{
    [Serializable]
    unsafe public struct Int2x24 : IBitNumberArray<int>, IEquatable<Int2x24>
    {
        internal UInt48 intern;


        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int2x24(int x0, int x1, int x2, int x3, int x4, int x5, int x6, int x7, int x8, int x9, int x10, int x11, int x12, int x13, int x14, int x15, int x16, int x17, int x18, int x19, int x20, int x21, int x22, int x23) : this(new int8(x0, x1, x2, x3, x4, x5, x6, x7), new int8(x8, x9, x10, x11, x12, x13, x14, x15), new int8(x16, x17, x18, x19, x20, x21, x22, x23))
        { }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int2x24(int x0_23)
        {
            int8 vector = x0_23;

            this = new Int2x24(vector, vector, vector);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int2x24(int8 x0_7, int8 x8_15, int8 x16_23)
        {
...
Assert.IsBetween(x16_23.x7, Int2.MinValue, Int2.MaxValue);
            ...
            int8 mask = (int)maxmath.bitmask32(new Int2x24().BitsPerNumber);

            intern = (UInt48)((ulong)(uint)maxmath.csum(maxmath.shl(mask & x0_7, new Int2x24().BitsPerNumber * new int8(0, 1, 2, 3, 4, 5, 6, 7))
                                                       |
                                                       maxmath.shl(mask & x8_15, new Int2x24().BitsPerNumber * new int8(8, 9, 10, 11, 12, 13, 14, 15)))
                             |
                             ((ulong)(uint)maxmath.csum(maxmath.shl(mask & x16_23, new Int2x24().BitsPerNumber * new int8(0, 1, 2, 3, 4, 5, 6, 7))) << (new Int2x24().BitsPerNumber * 16)));
        }
...
Assert.IsValidSubarray(index, numNumbers, Length);
            ...
            if (Constant.IsConstantExpression(value))
            {
                ...
                else
                {
                    ...
                    ulong newValues = new Int2x24(value).intern & mask;
