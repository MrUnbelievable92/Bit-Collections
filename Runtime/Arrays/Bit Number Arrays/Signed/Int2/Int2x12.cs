...
{
    [Serializable]
    unsafe public struct Int2x12 : IBitNumberArray<int>, IEquatable<Int2x12>
    {
        internal Int24 intern;


        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int2x12(int x0, int x1, int x2, int x3, int x4, int x5, int x6, int x7, int x8, int x9, int x10, int x11) : this(new int8(x0, x1, x2, x3, x4, x5, x6, x7), new int4(x8, x9, x10, x11))
        { }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int2x12(int x0_11)
        {
            int8 temp = x0_11;

            this = new Int2x12(temp, temp.v4_0);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int2x12(int8 x0_7, int4 x8_11)
        {
...
Assert.IsBetween(x8_11.x, Int2.MinValue, Int2.MaxValue);
Assert.IsBetween(x8_11.y, Int2.MinValue, Int2.MaxValue);
Assert.IsBetween(x8_11.z, Int2.MinValue, Int2.MaxValue);
Assert.IsBetween(x8_11.w, Int2.MinValue, Int2.MaxValue);

            int8 mask = (int)maxmath.bitmask32(new Int2x12().BitsPerNumber);

            intern = (Int24)(maxmath.csum(maxmath.shl(mask & x0_7, new Int2x12().BitsPerNumber * new int8(0, 1, 2, 3, 4, 5, 6, 7))
                                          |
                                          maxmath.shl(mask & new int8(x8_11, default(int4)), new Int2x12().BitsPerNumber * new int8(8, 9, 10, 11, 12, 13, 14, 15))));
        }
        ...
        public int this[[AssumeRange(0, 11)] int index]
        {
...
Assert.IsWithinArrayBounds(index, Length);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public int8 GetSubArray([AssumeRange(0, 4)] int index)
        {
Assert.IsValidSubarray(index, 8, Length);
            ...
            {
                // manual sign extend => 1 bitshift less; same if 'index' is not a compile time constant
                int8 temp = maxmath.shl((int8)(*(int*)ptr), 32 - (BitsPerNumber * (index + new int8(1, 2, 3, 4, 5, 6, 7, 8))));
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SetSubArray([AssumeRange(0, 4)] int index, int8 value)
        {
Assert.IsValidSubarray(index, 8, Length);
...
Assert.IsBetween(value.x7, MinValue, MaxValue);
            ...
            fixed (void* ptr = &this)
            {
                intern = (Int24)((int)maxmath.andnot(*(uint*)ptr,      maxmath.bitmask32(8 * BitsPerNumber,   index * BitsPerNumber))
                                 |
