...
namespace BitCollections
{
    unsafe internal static partial class BitAlgorithms
    {
        ...
        internal static int IndexOfFirst<T>(void* basePtr, int startIndex, int count, long value, int arrayLength, Comparison where)
            where T : BitInt
        {
...
Assert.IsWithinArrayBounds(startIndex + count - tobyte(count != 0), arrayLength);
            ...
            if (default(T).Bits == 1)
            {
                ...
                if (Hint.Likely(discardedBitsInFirstByte != 0))
                {
                    ...
                    int foundIndex = ((bit8)load).IndexOfFirst();

                    if (Hint.Unlikely((int)bitsToCheckInFirstByte >= count))
                    {
                        return (foundIndex >= count) ? -1
                                                     : foundIndex;
                    }
                    else
                    {
                        ...
                        bits += (int)bitsToCheckInFirstByte;
                        count -= (int)bitsToCheckInFirstByte;
                        basePtr = (byte*)basePtr + 1;
                    }
                }

                uint bytes = divrem((uint)count, 8u, out uint remBits);
                long byteIndex = Algorithms.SIMD_IndexOf((byte*)basePtr, bytes, (byte)(value - 1), Comparison.NotEqualTo);

                if (Hint.Unlikely(byteIndex == -1))
                {
                    if (Hint.Likely(remBits != 0))
                    {
                        bits += (int)(bytes * 8);
                        byte lastByte = *((byte*)basePtr + bytes);
                        int iof = ((value != 0 ? (bit8)lastByte : !(bit8)lastByte) & (bit8)bitmask8(remBits)).IndexOfFirst();

                        return iof == 32 ? -1 : bits + iof;
                    }
                }
                else
                {
                    bits += (int)(byteIndex * 8);
                    bit8 hit = ((bit8*)basePtr)[byteIndex];
                    int indexInLast = (value != 0 ? hit : !hit).IndexOfFirst();

                    return bits + indexInLast;
                }
            }
                  ...
                  || default(T).Bits == 4)
            {
                ...
                if (innerIndex != 0)
                {
                    ...
                    else
                    {
                        ...
                        if (cmp != 0)
                        {
                            return (int)((uint)tzcnt(cmp >> (default(T).Bits * innerIndex)) / default(T).Bits);
                        }
                        
                        count -= valuesToCheckInFirstByte;
                        idx += valuesToCheckInFirstByte;
                    }
                }
                ...
                while (count >= 256 / default(T).Bits)
                {
                    ...
                    if (!cmp.Equals(0))
                    {
                        int hitIdx = (int)((uint)__UInt256__.tzcnt(cmp.Reinterpret<uint8, __UInt256__>()) / default(T).Bits);
                        return idx + hitIdx;
                    }

                    count -= 256 / default(T).Bits;
                    idx += 256 / default(T).Bits;
                ...
                if (count >= 128 / default(T).Bits)
                {
                    ...
                    if (!cmp.Equals(0))
                    {
                        int hitIdx = (int)((uint)tzcnt(cmp.Reinterpret<uint4, UInt128>()) / default(T).Bits);
                        return idx + hitIdx;
                    }

                    count -= 128 / default(T).Bits;
                    idx += 128 / default(T).Bits;
                ...
                if (count >= 64 / default(T).Bits)
                {
                    ...
                    if (cmp != 0)
                    {
                        int hitIdx = (int)((uint)tzcnt(cmp) / default(T).Bits);
                        return idx + hitIdx;
                    }

                    count -= 64 / default(T).Bits;
                    idx += 64 / default(T).Bits;
